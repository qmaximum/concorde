...
static int norm = CC_EUCLIDEAN; //norm (must specify if dat file is not a TSPLIB file)
...
static int binary_in = 0; //dat file in binary doubles or int
static int binary_edges = 0; //edge files in binary
static int tsplib_in = 1; //输入的是TSPLibarary格式数据
static int nnodes_want = 0; //number of nodes for random problem 无输入数据生成k个点的随机问题
static int gridsize = 0; //use #x# grid for random points, no dups if #<0
static int nearnum = 0; //use #-nearest as the sparse edge set
static int quadtry = 2; //use quad #-nearest as the sparse set (default is 3)貌似这里的默认值是2
static int run_silently = 0; //run silently
static int kick_type = CC_LK_WALK_KICK; //kick (0-Random, 1-Geometric, 2-Close, 3-Random_Walk [default])
static int tour_type = LK_QBORUVKA; //generate starting cycle
                                    //(0-Rand, 1-NNeigh, 2-Greedy, 3-Boruvka, 4-QBoruvka[default])

static int in_repeater = -1; //number of kicks in iterated Lin-Kernighan (default is #nodes)
static int number_runs = 0; //number of runs
static double time_bound = -1.0; //running time bound in seconds
static double length_bound = -1.0; //tour length bound (stop when we hit d)

static char *nodefile = (char *) NULL; //输入文件名
static char *goodfname = (char *) NULL; //use the edges in file f as the sparse edge set
static char *cycfname = (char *) NULL; //starting cycle
static char *edgegenfname = (char *) NULL; //edgegen description file for the sparse edge set
static char *edgecycfname = (char *) NULL; //starting cycle (as an edgelist)
static char *saveit_final = (char *) NULL; //save final tour
static char *saveit_name = (char *) NULL; //save tour in f after every 10000 kicks
...
int main (int ac, char **av)
{
    ...
    CCutil_printlabel ();                //PRINTS information identifying a machine 输出系统信息
    CCutil_init_datagroup (&dat);        //initialize a CCdatagroup 初始化一个datagroup

    rval = print_command (ac, av);
    CCcheck_rval (rval, "print_command failed");

    seed = (int) CCutil_real_zeit ();    //To measure wall clock time 利用当前时间生成随机数种子
    if (parseargs (ac, av))
        return 1;
    CCutil_sprand (seed, &rstate);       //Call once to initialize the generator.初始化随机数发生器
    ...
    startzeit = CCutil_zeit ();          //To measure cpu time 开始记时

    if (tsplib_in) {
        if (CCutil_gettsplib (nodefile, &ncount, &dat)) {                  //读取TSPLibrary格式的数据
            fprintf (stderr, "could not read the TSPLIB file\n");
            rval = 1;
            goto CLEANUP;
        }
        CCutil_dat_getnorm (&dat, &norm);                                  //get the norm of a CCdatagroup
    } else {
        ncount = nnodes_want;
        if (gridsize < 0) {
        ...
        if (CCutil_getdata (nodefile, binary_in, norm, &ncount, &dat,      //从数据文件中读取数据
    ...
    incycle = CC_SAFE_MALLOC (ncount, int);               //a macro (type *) CCutil_allocrus (((size_t) (nnum)) * sizeof (type))
                                                          //宏无法从库中导出因此在python中实现
                                                          //allocrus = libc.CCutil_allocrus
                                                          //incycle = POINTER(c_int)() 生成空指针 pointer(c_int())则是初始化了一个指向0的实例
